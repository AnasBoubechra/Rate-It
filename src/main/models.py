...
from ..users import AdminRequired
...
class ContactUs(db.Model):
    ...
    deck_id = Column(db.Integer, nullable=True)
...
class ContactUsView(AdminRequired, sqla.ModelView):
    column_sortable_list = ('id', 'name', 'email', 'received_time')
    column_filters = ('id', 'name', 'email', 'received_time')

    def __init__(self, session):
        super(ContactUsView, self).__init__(ContactUs, session)
